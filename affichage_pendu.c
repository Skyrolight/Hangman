#include "client.h"
#include "serveur.h"

void affPendu(int nbErreur) { 
	switch(nbErreur) {
		case 0 :
			printf("             ooooooooooooooooooooooo         \n");
			printf("            /00000000000000000000000         \n");
			printf("            00               0    00         \n");
			printf("            00                0   00         \n");
			printf("            00                 0  00         \n");
			printf("                                0 00         \n");
			printf("                                 000         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                           d00000000000000b  \n");
			printf("                          doooooooooooooooob \n");

			break;

		case 1 :
			printf("             ooooooooooooooooooooooo         \n");
			printf("            /00000000000000000000000         \n");
			printf("            00               0    00         \n");
			printf("            00                0   00         \n");
			printf("            00                 0  00         \n");
			printf("             |                  0 00         \n");
			printf("             |                   000         \n");
			printf("             |                    00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                           d00000000000000b  \n");
			printf("                          doooooooooooooooob \n");

			break;

		case 2:
			printf("             ooooooooooooooooooooooo         \n");
			printf("            /00000000000000000000000         \n");
			printf("            00               0    00         \n");
			printf("            00                0   00         \n");
			printf("            00                 0  00         \n");
			printf("             |                  0 00         \n");
			printf("             |                   000         \n");
			printf("             |                    00         \n");
			printf("            ooo                   00         \n");
			printf("           88888                  00         \n");
			printf("            888                   00         \n");
			printf("             l                    00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                           d00000000000000b  \n");
			printf("                          doooooooooooooooob \n");

			break;

		case 3:
			printf("             ooooooooooooooooooooooo         \n");
			printf("            /00000000000000000000000         \n");
			printf("            00               0    00         \n");
			printf("            00                0   00         \n");
			printf("            00                 0  00         \n");
			printf("             |                  0 00         \n");
			printf("             |                   000         \n");
			printf("             |                    00         \n");
			printf("            ooo                   00         \n");
			printf("           88888                  00         \n");
			printf("            888                   00         \n");
			printf("             l                    00         \n");
			printf("         @8555558@                00         \n");
			printf("          |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                           d00000000000000b  \n");
			printf("                          doooooooooooooooob \n");

			break;

		case 4:
			printf("             ooooooooooooooooooooooo         \n");
			printf("            /00000000000000000000000         \n");
			printf("            00               0    00         \n");
			printf("            00                0   00         \n");
			printf("            00                 0  00         \n");
			printf("             |                  0 00         \n");
			printf("             |                   000         \n");
			printf("             |                    00         \n");
			printf("            ooo                   00         \n");
			printf("           88888                  00         \n");
			printf("            888                   00         \n");
			printf("             l                    00         \n");
			printf("         @8555558@                00         \n");
			printf("          |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("           @@                     00         \n");
			printf("          @@                      00         \n");
			printf("         @@                       00         \n");
			printf("        @@                        00         \n");
			printf("       @@                         00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                           d00000000000000b  \n");
			printf("                          doooooooooooooooob \n");

			break;

		case 5:
			printf("             ooooooooooooooooooooooo         \n");
			printf("            /00000000000000000000000         \n");
			printf("            00               0    00         \n");
			printf("            00                0   00         \n");
			printf("            00                 0  00         \n");
			printf("             |                  0 00         \n");
			printf("             |                   000         \n");
			printf("             |                    00         \n");
			printf("            ooo                   00         \n");
			printf("           88888                  00         \n");
			printf("            888                   00         \n");
			printf("             l                    00         \n");
			printf("         @8555558@                00         \n");
			printf("          |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("           @@@@                   00         \n");
			printf("          @@  @@                  00         \n");
			printf("         @@    @@                 00         \n");
			printf("        @@      @@                00         \n");
			printf("       @@        @@               00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                           d00000000000000b  \n");
			printf("                          doooooooooooooooob \n");

			break;

		case 6:
			printf("             ooooooooooooooooooooooo         \n");
			printf("            /00000000000000000000000         \n");
			printf("            00               0    00         \n");
			printf("            00                0   00         \n");
			printf("            00                 0  00         \n");
			printf("             |                  0 00         \n");
			printf("             |                   000         \n");
			printf("             |                    00         \n");
			printf("            ooo                   00         \n");
			printf("           88888                  00         \n");
			printf("            888                   00         \n");
			printf("             l                    00         \n");
			printf("         @8555558@                00         \n");
			printf("        @ |55555|                 00         \n");
			printf("       @  |55555|                 00         \n");
			printf("      @   |55555|                 00         \n");
			printf("     @    |55555|                 00         \n");
			printf("          |55555|                 00         \n");
			printf("           @@@@                   00         \n");
			printf("          @@  @@                  00         \n");
			printf("         @@    @@                 00         \n");
			printf("        @@      @@                00         \n");
			printf("       @@        @@               00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                           d00000000000000b  \n");
			printf("                          doooooooooooooooob \n");

			break;

		case 7:
			printf("             ooooooooooooooooooooooo         \n");
			printf("            /00000000000000000000000         \n");
			printf("            00               0    00         \n");
			printf("            00                0   00         \n");
			printf("            00                 0  00         \n");
			printf("             |                  0 00         \n");
			printf("             |                   000         \n");
			printf("             |                    00         \n");
			printf("            ooo                   00         \n");
			printf("           88888                  00         \n");
			printf("            888                   00         \n");
			printf("             l                    00         \n");
			printf("         @8555558@                00         \n");
			printf("        @ |55555| @               00         \n");
			printf("       @  |55555|  @              00         \n");
			printf("      @   |55555|   @             00         \n");
			printf("     @    |55555|    @            00         \n");
			printf("          |55555|                 00         \n");
			printf("           @@@@                   00         \n");
			printf("          @@  @@                  00         \n");
			printf("         @@    @@                 00         \n");
			printf("        @@      @@                00         \n");
			printf("       @@        @@               00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                                  00         \n");
			printf("                           d00000000000000b  \n");
			printf("                          doooooooooooooooob \n");

			break;

		case 8:
			printf("                                                      \n");
			printf(" .______    _______ .______       _______   __    __  \n");
			printf(" |   _  |  |   ____||   _  |     |       | |  |  |  | \n");
			printf(" |  |_)  | |  |__   |  |_)  |    |  .--.  ||  |  |  | \n");
			printf(" |   ___/  |   __|  |      /     |  |  |  ||  |  |  | \n");
			printf(" |  |      |  |____ |  ||  |----.|  '--'  ||  `--'  | \n");
			printf(" | _|      |_______|| _| `._____||_______/  |______/  \n");
			printf("                                                      \n");
		break;

		case 9:
			printf("                                                              \n");
			printf("   _______      ___        _______ .__   __.  _______     __  \n");
			printf("  /  _____|    /   |      /  _____||  | |  | |   ____|   |  | \n");
			printf(" |  |  __     /  ^  |    |  |  __  |   ||  | |  |__      |  | \n");
			printf(" |  | |_ |   /  /_|  |   |  | |_ | |  . `  | |   __|     |  | \n");
			printf(" |  |__| |  /  _____  |  |  |__| | |  ||   | |  |____    |__| \n");
			printf("  |______| /__/     |__|  |______| |__| |__| |_______|   (__) \n");
			printf("                                                              \n");
		break;
	}
}